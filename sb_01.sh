...
index_url="https://github.com/ocarmelo/sbox/releases/download/00/index.html"
...
quick_command() {
  ...
  echo "bash <(curl -Ls https://raw.githubusercontent.com/ocarmelo/sbox/test/sb_01.sh)" >> "$SCRIPT_PATH"
  chmod +x "$SCRIPT_PATH"
  if [[ ":$PATH:" != *":$HOME/bin:"* ]]; then
      echo "export PATH=\"\$HOME/bin:\$PATH\"" >> "$HOME/.bashrc"
      source "$HOME/.bashrc"
  fi
green "快捷指令01创建成功,下次运行输入01快速进入菜单\n"
}
