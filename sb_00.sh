...
if [[ $tcp_ports -ne 1 || $udp_ports -ne 1 ]]; then  # 修改UDP端口需求为1
    red "端口规则不符合要求，正在调整..."
    ...
    if [[ $udp_ports -gt 1 ]]; then  # 只保留1个UDP端口
    ...
    if [[ $udp_ports -lt 1 ]]; then  # 只添加1个UDP端口
        udp_ports_to_add=1
        udp_ports_added=0
...
purple "hy2使用的UDP端口: $udp_port1"  # 删除tuic端口提示
export VLESS_PORT=$tcp_port
export HY2_PORT=$udp_port1  # 移除TUIC_PORT变量
}
...
changge_ports() {
reading "将删除全部端口然后随机开放1个tcp端口和1个udp端口,确定继续吗?(直接回车即确认更换)【y/n】: " choice  # 修改提示信息
...
echo -e "${yellow}本脚本同时双协议共存${purple}(vless-reality,hysteria2)${re}"  # 修改协议描述
...
quick_command() {
  ...
  echo "bash <(curl -Ls https://raw.githubusercontent.com/ocarmelo/sbox/main/sb_00.sh)" >> "$SCRIPT_PATH"
  chmod +x "$SCRIPT_PATH"
...
restart_singbox() {
  if [[ -e $WORKDIR/config.json ]]; then
    yellow "正在重启sing-box主进程..."
    cd $WORKDIR
    pgrep -x "$(basename ${FILE_MAP[web]})" | xargs -r kill -9 >/dev/null 2>&1
    nohup ./"$(basename ${FILE_MAP[web]})" run -c config.json >/dev/null 2>&1 &
    sleep 2
...
menu() {
  ...
  echo -e "${green}脚本地址：${re}${yellow}https://github.com/ocarmelo/sbox${re}\n"
