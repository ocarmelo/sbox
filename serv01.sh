  ...
  reading "\n确定要卸载吗？【y/n】: " choice
    case "$choice" in
	  ...
	  #crontab -l | grep -v "serv00keep" >rmcron
          ...
          purple "欢迎继续使用脚本：bash <(curl -Ls https://raw.githubusercontent.com/ocarmelo/sbox/main/serv01.sh)"
          purple "************************************************************"
	  curl -sSL https://raw.githubusercontent.com/ocarmelo/sbox/main/serv01.sh -o serv01.sh && chmod +x serv01.sh
          ;;
        [Nn]) exit 0 ;;
...
if [[ -e $WORKDIR/config.json ]]; then
  ...
  mkdir -p "$HOME/bin"
  curl -Ls https://raw.githubusercontent.com/ocarmelo/sbox/main/serv01.sh > "$SCRIPT_PATH"
  chmod +x "$SCRIPT_PATH"
