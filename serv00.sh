  ...
  reading "\n确定要卸载吗？【y/n】: " choice
    case "$choice" in
	  ...
	  bash -c 'ps aux | grep $(whoami) | grep -v "sshd\|bash\|grep" | awk "{print \$2}" | xargs -r kill -9 >/dev/null 2>&1' >/dev/null 2>&1
          ...
          purple "************************************************************"
	  curl -sSL https://raw.githubusercontent.com/ocarmelo/sbox/main/serv00.sh -o serv00.sh && chmod +x serv00.sh
          ;;
        [Nn]) exit 0 ;;
...
echo -e "当前 Serv00 脚本最新版：${purple}${insV}${re} (已安装)"
else
echo -e "当前 Serv00 脚本版本号：${purple}${insV}${re}"
echo -e "检测到最新 Serv00 脚本版本号：${yellow}${latestV}${re} (可选择4进行更新)"
...
echo -e "当前 Serv00 脚本版本号：${purple}${latestV}${re}"
